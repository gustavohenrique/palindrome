...
gulp.task('copy-html', function () {
    ...
    var sources = gulp.src(['app/mocks/env.js'], { read: false });
 
    return target.pipe($.inject(sources, { relative: true })).pipe(gulp.dest('.tmp'));
});

gulp.task('mocks', function () {
    return gulp.src('app/mocks/*').pipe(gulp.dest('.tmp/mocks'));
});
...
gulp.task('serve', ['compile-scripts', 'mocks', 'connect', 'watch'], function () {
